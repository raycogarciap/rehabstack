...
// Debe ser "use client" porque usa usePathname() para detectar la ruta activa
// (los layouts Server Component no re-renderizan en navegación, per docs de Next.js 15).
// También gestiona el toggle del menú móvil con useState.
...
// Definición de los enlaces de navegación del dashboard
const NAV_LINKS = [
  {
    label: "Dashboard",
    href: "/dashboard",
    // Icono SVG inline: cuadrícula 2×2
    icon: (
...
export function SidebarNav({ userEmail }: SidebarNavProps) {
  ...
  // /dashboard es activo solo con match exacto para evitar que aparezca activo en subrutas
  const isActive = (href: string) =>
    href === "/dashboard" ? pathname === href : pathname.startsWith(href);

  // Clases comunes para cada enlace de nav
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex fixed left-0 top-0 z-40 h-screen w-60 flex-col border-r border-neutral-200 bg-white">
        ...
        {/* Footer del sidebar: email del usuario y botón de logout */}
