// src/components/creator/new-agent-form.tsx
// Formulario de 5 pasos para publicar un nuevo agente en el marketplace RehabStack.
// Client Component: maneja estado React para navegación entre pasos, borradores
// en localStorage y envío al API /api/creator/agents.
...
// Forma completa del formulario del agente — refleja la tabla agents de Supabase
...
const INITIAL_FORM: AgentFormData = {
  ...
  min_model_capability: 'cloud_only',
  quick_actions: [],
  work_item_types_text: '',
  staff_delegation: false,
  relevant_integrations: [],
  knowledge_base: '',
  test_scenarios: [{ user_message: '', expected_behavior: '' }],
  known_limitations: '',
  compliance_declaration: false,
}

// Clave de localStorage para guardar borradores del formulario
const DRAFT_KEY = 'creator-agent-form-draft'
          ...
          <SelectContent>
            {CATEGORY_OPTIONS.map((cat) => (
              <SelectItem key={cat} value={cat}>
                {/* Convierte kebab-case a Title Case para mostrar */}
...
function Step3({ form, updateForm, t }: Step3Props) {
  // Genera un ID único para una nueva acción rápida
  const uid = useId()
...
function Step4({ form, updateForm, t }: Step4Props) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-3">
        ...
        {form.test_scenarios.length < 3 && (
          <p className="text-xs text-amber-600 font-medium">
            ⚠ Al menos 3 escenarios son requeridos antes de enviar.
          </p>
        )}
      </div>
      ...
      {/* Declaración de cumplimiento — obligatoria para enviar */}
...
function Step5({ form, t, onSubmit, submitting }: Step5Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-xs text-neutral-500 text-center">{t('submitDesc')}</p>

      {/* Botón de envío */}
      <Button
        type="button"
        onClick={onSubmit}
        disabled={submitting || !form.compliance_declaration || form.test_scenarios.length < 3}
        className="w-full"
      >
      ...
      {!form.compliance_declaration && (
        <p className="text-xs text-red-600 text-center">
          Debes marcar la declaración de cumplimiento en el Paso 4 para enviar.
        </p>
      )}
    </div>
  )
...
export function NewAgentForm() {
  ...
  // Router para navegación programática tras el envío
  ...
  // Persiste el borrador en localStorage cada vez que cambia el formulario
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        // Ignorar errores de JSON malformado
      }
    }
  }, [])
  ...
  // Envía el formulario al API route del creador
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Limpia el borrador local y muestra el estado de éxito
      localStorage.removeItem(DRAFT_KEY)
      setSubmitted(true)
    } catch (e) {
      console.error('Error al enviar el agente:', e)
      alert('Error al enviar el agente. Por favor, inténtalo de nuevo.')
    } finally {
      setSubmitting(false)
    }
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center gap-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-neutral-600 max-w-sm">{t('successMessage')}</p>
        </div>
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mb-8">
        ...
        {step === 5 && (
          <Step5 form={form} t={t} onSubmit={handleSubmit} submitting={submitting} />
        )}
      </div>
      ...
      <div className="flex items-center justify-between border-t border-neutral-200 pt-6">
        ...
        {/* Botón Siguiente — oculto en el paso 5 (donde está el botón de Submit) */}
        {step < 5 ? (
